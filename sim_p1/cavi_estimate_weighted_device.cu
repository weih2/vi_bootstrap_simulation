...
void cavi_implementation::device_cavi_estimate_weighted(int thread_id){
  ...
  for(int i = 0; i < *device_g_vars.device_n_samples; i++){
    sum_phi = 0;
    weight_index = (*device_n_bootstrap_samples) * thread_id + i;
    ...
    for(int k = 0; k < *device_g_vars.device_K; k++){
      phi_index = thread_id
        ...
        * (*device_g_vars.device_K)
        + k;
      par_index = thread_id * (*device_g_vars.device_K) + k;
  ...
  for(int k = 0; k < *device_g_vars.device_K; k++){
    ...
    for(int i = 0; i < data.g_vars.n_samples; i++){
      ...
      device_elbo[thread_id] += device_est.device_phi[phi_index] * device_weights[weight_index]
...
void cavi_implementation::device_cavi_bootstrap_update_single(){
  ...
  if(thread_id >= *device_n_bootstrap_samples) return;

  const int par_index_start = thread_id * (*device_g_vars.device_K);
  const int par_index_end = (thread_id + 1) * (*device_g_vars.device_K);

  device_generate_weights((*device_exp_id), thread_id);
  double device_old_elbo;

  for(int n_iter = 0; n_iter < *device_max_n_iter; n_iter++){
    device_old_elbo = device_elbo[thread_id];
    device_cavi_estimate_weighted(thread_id);
  ...
  for(int par_index = 0; par_index < (*device_g_vars.device_K); par_index++){
    device_est.device_m_transpose[+ thread_id]
