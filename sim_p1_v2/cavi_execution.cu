__global__ void cavi_execute(bridge bg, int bootstrap_execution){
  ...
  if(bootstrap_execution){
    ...
    for(int k = 0; k < K; k++){
      if((thread_implementation.vp_cs[0] < thread_implementation.mu[k])
        &&(thread_implementation.vp_cs[1] > thread_implementation.mu[k]))
        device_vp_cs_covered[k * n_experiments + t_id]++;
      if((thread_implementation.vwlb_cs[0] < thread_implementation.mu[k])
        &&(thread_implementation.vwlb_cs[1] > thread_implementation.mu[k]))
