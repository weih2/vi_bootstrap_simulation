...
int main(){
    ...
    *dev_settings.ci_quantile = cdf_ugaussian_Pinv(0.975);
    ...
    double vwlb_cs_covered_counts_total[DELTA_COUNT][N_CLUSTERS];
    double vp_cs_covered_counts_total[DELTA_COUNT][N_CLUSTERS];
    double empirical_ci_covered_counts_total[DELTA_COUNT][N_CLUSTERS];
    double vwlb_cs2_covered_counts_total[DELTA_COUNT][N_CLUSTERS];
    ...
    for(int delta_count = 0; delta_count < DELTA_COUNT; delta_count++){
      ...
      for(int k = 0; k < N_CLUSTERS; k++){
        vwlb_cs_covered_counts_total[delta_count][k] = 0;
        vp_cs_covered_counts_total[delta_count][k] = 0;
        empirical_ci_covered_counts_total[delta_count][k] = 0;
        vwlb_cs2_covered_counts_total[delta_count][k] = 0;
      }

      n_outliers[delta_count] = bridge_0.n_outliers;

      for(int k = 0; k < N_CLUSTERS; k++){
        vwlb_cs_covered_counts_total[delta_count][k]
         = bridge_0.vwlb_cs_covered_counts[k];
        vp_cs_covered_counts_total[delta_count][k]
         = bridge_0.vp_cs_covered_counts[k];
       vwlb_cs2_covered_counts_total[delta_count][k]
         = bridge_0.vwlb_cs2_covered_counts[k];
        empirical_ci_covered_counts_total[delta_count][k]
    ...
    for(int delta_count = 0; delta_count < DELTA_COUNT; delta_count++){
      ...
      for(int k = 0; k < N_CLUSTERS; k++){
        vwlb_cs_covered_counts_total[delta_count][k] /= double(n_effective);
        vp_cs_covered_counts_total[delta_count][k] /= double(n_effective);
        empirical_ci_covered_counts_total[delta_count][k] /= double(n_effective);
        vwlb_cs2_covered_counts_total[delta_count][k] /= double(n_effective);
      }
    }

    for(int k = 0; k < N_CLUSTERS; k++){
      for(int delta_count = 0; delta_count < DELTA_COUNT; delta_count++){
        printf("%f ", vwlb_cs_covered_counts_total[delta_count][k]);
      }
      printf("\n");
    }
    ...
    for(int k = 0; k < N_CLUSTERS; k++){
      for(int delta_count = 0; delta_count < DELTA_COUNT; delta_count++){
        printf("%f ", vwlb_cs2_covered_counts_total[delta_count][k]);
      }
      printf("\n");
    }
    ...
    for(int k = 0; k < N_CLUSTERS; k++){
      for(int delta_count = 0; delta_count < DELTA_COUNT; delta_count++){
        printf("%f ", vp_cs_covered_counts_total[delta_count][k]);
      }
      printf("\n");
    }
